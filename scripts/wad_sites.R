...
data = read_tsv("data/Aselloidea/Aselloidea_All_Occurences_Europe_DarCo.tsv", col_names = T, quote = "\"", locale = locale(decimal_mark = ",")) %>%
  mutate(Country = str_to_lower(Country)) %>%
  left_join(countries, by = "Country")

data %>% filter(is.na(CountryCode)) %>% select(Country)
